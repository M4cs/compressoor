...
class EnvelopeTests(unittest.TestCase):
    ...
    def test_skill_defaults_require_terse_execution_mode(self) -> None:
        ...
        self.assertIn("emit only the actual encoder-produced packed `CCM1` or compact-template payload", skill)
        self.assertIn("produce it with the compressoor encoder rather than writing plaintext", skill)
        ...
        self.assertIn("Never send a progress update unless the environment requires one.", skill)
        self.assertIn("Never send a human-readable pre-tool status message", skill)
        self.assertIn("Never hand-write a fake packed status; use the encoder output when a progress update is unavoidable.", skill)
        self.assertIn("Never bypass compressoor for reusable memory, handoffs, or required progress payloads.", skill)
